...
# 
geocubes = sorted((basepath / 'regmaps_native').glob('*.cub'))
...
ortho_prj_path = basepath / 'orthographic_projection_native'
...
equitiff_path = basepath / 'Equirectangular_native_tif'
os.makedirs(equitiff_path, exist_ok=True)

orthoind_path = basepath / 'Orthographic_native_tif'
...
for geocub in geocubes:
    ...
    command = 'gdal_translate  -ot UINT32 -r cubicspline -of GTiff {} {}'.format(geocub, equitiff_path.joinpath("{}.tif".format(geocub.stem)))
    # -scale -0.5 5 0 1
    print(command)
    os.system(command)   
