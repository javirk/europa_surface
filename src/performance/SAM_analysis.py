...
import numpy as np
from torchmaskrcnn import get_model_instance_segmentation
import time
...
import sys
sys.path.append((titaniach / 'lineament_detection/pytorch_maskrcnn/code/vision/references/detection').as_posix())

from engine import evaluate_fromoutput
...
# define segmentation and mask plotting routine
def segmask_box(img, target, threshold=0.5, score_thresh=0.5, alpha=0.8, colors=None, width=1, fontsize=7, font=None, del_pxs=10, del_boxes=True):
    """
    ...
    for bbox, color, label in zip(img_boxes, colors, labels):  # type: ignore[arg-type]
        ...
        if label is not None:
            ...
            margin = width + 1
            try:
                draw.text((bbox[0] + margin, bbox[1] + margin), str(label), fill=(0,0,0), font=txt_font) # text in black
...
datafolders = []
# new test set, 112x112
# datafolders.append("2024_02_14_11_22_Regiomaps_112x112") --> does not make sense, I don't have results from SAM
# new test set, 224x224
datafolders.append("2023_10_16_15_34_Regiomaps_224x224") # correct comparison for SAM
# old test set, 224x224
# First, shift from 5 to 4. 
datafolders.append("val_and_train_pytorch_224x224_LINEAMAPPER_v1.0")
#old test set, re-tiled to 112x112
# "val_and_train_pytorch_112x112_LINEAMAPPER_v1.0"

model_names = []
runs = []
runs.append('run01')
model_names.append("SAM_v1") # LineaMapper v1.0
...
for run, model_name in zip(runs, model_names):
    for datafolder in datafolders:
        ...
        # TODO: select or toggle
        # %matplotlib inline
        save=True
        evaluate_now=True
        # choose threshold!
        # should be 0.5 to be consistent with metrics...
        mask_threshold = 0.5
        ...
        # and evaluates metrics on them if chosen (these get saved in the home directory, because I needed to implement it right in ...cocoeval.py source code)
        ...
        hdfpaths_orig = Path('Z:\Groups\PIG\Caroline\lineament_detection\Reinforcement_Learning_SAM/test_SAMv1') 
        # hdfpaths = sorted(hdfpath.glob('*.hdf5'))
        ...
        model_output = hdf_to_output(hdfpaths_orig, dataloader_val) # 
        # check length:
        ...
        for idx in range(len(dataset)): # len(dataset)
            ...
            t, masks = segmask_box(img, prediction,threshold=mask_threshold, score_thresh=score_thresh, alpha=0.8, colors=None, width=1, fontsize=6, font=None, del_boxes=False)
            # ground truth:
            tgr, masks_gr = segmask_box(img, target, threshold=None, alpha=0.8, colors=None, width=1, fontsize=6, font=None, del_boxes=False)
            ...
            if save:
                ...
                if pdf:
                    ...
                    fig.savefig(save_path_pdf.joinpath(target['path'].stem + '_' + figsubset + '_' + str(idx) + run + '_score_thr_' + str(score_thresh) + '_mask_thr_' + str(mask_threshold) + '.pdf'), bbox_inches = 'tight', facecolor='white')
                # png folder
                if png:
                    save_path_png = basepath / 'output/maskrcnn' / (dt_ymd + run) / 'png'
                    os.makedirs(save_path_png, exist_ok=True)
                    fig.savefig(save_path_png.joinpath(target['path'].stem + '_' + figsubset + '_' + str(idx) + run + '.png'), bbox_inches = 'tight', facecolor='white') # to make it shorter: '_threshold_' + str(score_thresh) + '_mask_thr_' + str(mask_threshold) + 
