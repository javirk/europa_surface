...
for tifffile in tifpaths:
    ...
    # for normal: command = 'python LineaMapper_to_img.py --geofile=z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/data/for_analysis/geotiffs/original_ArcGIS_export_basemap/' + tf + '.tif --savedir=' + savepath + ' --geosize=112 --cut_size=546 --class_scores 0.5 0.5 0.5 0.5 --azimuth_diff_range=5 --multiplication_factor=6'
    # for lower-res areas: geosize=500
    command = 'python LineaMapper_v2_to_img.py --geofile=z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/data/for_analysis/geotiffs/Europa/' + tf + '.tif --savedir=' + savepath + ' --geosize=' + str(geosize) + ' --cut_size=2500 --class_scores 0.9 0.4 0.9 0.4 --azimuth_diff_range=25 --del_pxs=100 --multiplication_factor=25'
