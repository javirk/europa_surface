...
# get path to basemap
basepath_data = titaniach / 'lineament_detection/global_maps_for_book/data/for_analysis/geotiffs/Europa/'
tifpaths = sorted(basepath_data.glob('*.tif'), reverse=True) # the reverse makes that equirect is executed first
...
for tifffile in tifpaths:
    ...
    savepath = 'z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/output/LineaMapper/' + dt_string + tf + f'geosize_{geosize}'
    # savepath = 'z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/output/LineaMapper/2024_06_02_10_07equirect_ngeosize_112'
    ...
    command = 'python LineaMapper_v2_to_img.py --geofile=z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/data/for_analysis/geotiffs/Europa/' + tf + '.tif --savedir=' + savepath + ' --geosize=' + str(geosize) + ' --cut_size=546 --class_scores 0.4 0.4 0.4 0.4 --azimuth_diff_range=25 --del_pxs=100 --multiplication_factor=25'
    # for lower-res areas: geosize=500
    # command = 'python LineaMapper_v2_to_img.py --geofile=z:/Groups/PIG/Caroline/lineament_detection/global_maps_for_book/data/for_analysis/geotiffs/original_ArcGIS_export_basemap/' + tf + '.tif --savedir=' + savepath + f' --geosize={geosize} --cut_size=2500 --class_scores 0.9 0.4 0.9 0.4 --azimuth_diff_range=15 --multiplication_factor=9'
    ...
    for geoi, geojfile in enumerate(geojfiles):
        ...
        # we need the first saving so we can append this later!
        command = 'ogr2ogr -nlt POLYGON -skipfailures {} {}'.format(savepath + '/shape_files/' + geojfile.stem + '.shp', savepath + '/json_files/' + geojfile.stem + '.geojson')
        print(command)
        os.system(command)
